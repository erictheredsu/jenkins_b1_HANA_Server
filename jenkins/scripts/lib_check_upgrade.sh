...
#UPGRADER_POOL=/home/jenkins/buildspace/upgrader/
...
BRANCH_POOL=${UPGRADER_POOL%/}/$BRANCH
latest=""
tested_cl=""
extract_pkg_name=""
NEW_BUILD_PATH=""
...
function findLatest() {
	...
	HAS_NEW_BUILD=true
    PKG_PATH=$(ls -t "${BRANCH_PATH%/}"/*$LATEST_CL.HANA/Upgrade_*.rar | awk "NR==1" | awk '{ print $1 }')
	CLIENT_PATH=$(ls -t "${PKG_PATH%Upgrade_*}"Client_*.zip | awk "NR==1" | awk '{ print $1 }')
	extract_pkg_name=`echo ${PKG_PATH%/Upgrade_*} | sed s:$BRANCH_PATH::g`
	...
	if [ ! -f $PKG_PATH ]; then
		echo "######################### Cannot find the Upgrade package"
		exit 1
	fi
...
function findExact() {
    ...
    PKG_PATH=$(ls "${BRANCH_PATH%/}"/$BUILD_PKG/Upgrade_*.rar | awk "NR==1" | awk '{ print $1 }')
